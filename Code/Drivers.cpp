...
void Drivers_RBGLED::Drivers_RBGLED_xxx(uint16_t Duration, uint8_t Traversal_Number, CRGB colour)
{
...
void Drivers_RBGLED::Drivers_RBGLED_Init(uint8_t set_Brightness)
{
  FastLED.addLeds<NEOPIXEL, PIN_RBGLED>(leds, NUM_LEDS);
  FastLED.setBrightness(set_Brightness);
}
#if _Test_Drivers
void Drivers_RBGLED::Drivers_RBGLED_Test(void)
{
  leds[0] = CRGB::White;
  FastLED.show();
  delay_xxx(50);
  ...
  FastLED.show();
  delay_xxx(50);
  Drivers_RBGLED_xxx(50 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Black);
}
#endif

void Drivers_RBGLED::Drivers_RBGLED_Color(uint8_t LED_s, uint8_t r, uint8_t g, uint8_t b, uint8_t p)
{
  if (LED_s > NUM_LEDS)
    return;

  if (FastLED.getBrightness() != p)
...
/*Key*/
uint8_t Drivers_Key::keyValue = 0;
...
void Drivers_Key::Drivers_Key_Init(void)
{
...
#if _Test_Drivers
void Drivers_Key::Drivers_Key_Test(void)
{
  Serial.println(Drivers_Key::keyValue);
}
#endif

void Drivers_Key::Drivers_key_Get(uint8_t *get_keyValue)
{
  *get_keyValue = keyValue;
}

/*ITR20001 Detection*/
bool Drivers_ITR20001::Drivers_ITR20001_Init(void)
{
...
int Drivers_ITR20001::Drivers_ITR20001_getAnaloguexxx_L(void)
{
  return analogRead(PIN_ITR20001xxxL);
}
int Drivers_ITR20001::Drivers_ITR20001_getAnaloguexxx_M(void)
{
  return analogRead(PIN_ITR20001xxxM);
}
int Drivers_ITR20001::Drivers_ITR20001_getAnaloguexxx_R(void)
{
  return analogRead(PIN_ITR20001xxxR);
}
#if _Test_Drivers
void Drivers_ITR20001::Drivers_ITR20001_Test(void)
{
  Serial.print("\tL=");
...
/*Voltage Detection*/
void Drivers_Voltage::Drivers_Voltage_Init(void)
{
  pinMode(PIN_Voltage, INPUT);
  // analogReference(INTERNAL);
}
float Drivers_Voltage::Drivers_Voltage_getAnalogue(void)
{
...
#if _Test_Drivers
void Drivers_Voltage::Drivers_Voltage_Test(void)
{
  // float Voltage = ((analogRead(PIN_Voltage) * 5.00 / 1024) * 7.67); //7.66666=((10 + 1.50) / 1.50)
...
#endif
/*Motor control*/
void Drivers_Motor::Drivers_Motor_Init(void)
{
...
#if _Test_Drivers
void Drivers_Motor::Drivers_Motor_Test(void)
{
  // A...Right
  // B...Left
...
*/
void Drivers_Motor::Drivers_Motor_control(boolean direction_A, uint8_t speed_A, // Group A motor parameters
...
void Drivers_ULTRASONIC::Drivers_ULTRASONIC_Init(void)
{
  pinMode(ECHO_PIN, INPUT); // Ultrasonic module initialization
  pinMode(TRIG_PIN, OUTPUT);
}
void Drivers_ULTRASONIC::Drivers_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get /*out*/)
{
...
#if _Test_Drivers
void Drivers_ULTRASONIC::Drivers_ULTRASONIC_Test(void)
{
...
void Drivers_Servo::Drivers_Servo_Init(unsigned int Position_angle)
{
...
#if _Test_Drivers
void Drivers_Servo::Drivers_Servo_Test(void)
{
  for (;;)
  {
    myservo.attach(PIN_Servo_z);
...
void Drivers_Servo::Drivers_Servo_control(unsigned int Position_angle)
{
  myservo.attach(PIN_Servo_z);
...
void Drivers_Servo::Drivers_Servo_controls(uint8_t Servo, unsigned int Position_angle)
{
...
void Drivers_IRrecv::Drivers_IRrecv_Init(void)
{
  irrecv.enableIRIn(); // Enable infrared communication NEC
}
bool Drivers_IRrecv::Drivers_IRrecv_Get(uint8_t *IRrecv_Get /*out*/)
{
...
#if _Test_Drivers
void Drivers_IRrecv::Drivers_IRrecv_Test(void)
{
  if (irrecv.decode(&results))
  {
