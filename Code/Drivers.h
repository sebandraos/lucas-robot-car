...
#define _Test_Drivers 0

/*RBG LED*/
#include "FastLED.h"
class Drivers_RBGLED
{
public:
  void Drivers_RBGLED_Init(uint8_t set_Brightness);
  void Drivers_RBGLED_xxx(uint16_t Duration, uint8_t Traversal_Number, CRGB colour);
#if _Test_Drivers
  void Drivers_RBGLED_Test(void);
#endif
  void Drivers_RBGLED_Color(uint8_t LED_s, uint8_t r, uint8_t g, uint8_t b, uint8_t p);
...
{
public:
  void Drivers_Key_Init(void);
#if _Test_Drivers
  void Drivers_Key_Test(void);
#endif
  void Drivers_key_Get(uint8_t *get_keyValue);
...
/*ITR20001 Detection*/
class Drivers_ITR20001
{
public:
  bool Drivers_ITR20001_Init(void);
  int Drivers_ITR20001_getAnaloguexxx_L(void);
  int Drivers_ITR20001_getAnaloguexxx_M(void);
  int Drivers_ITR20001_getAnaloguexxx_R(void);
#if _Test_Drivers
  void Drivers_ITR20001_Test(void);
#endif
...
/*Voltage Detection*/
class Drivers_Voltage
{
public:
  void Drivers_Voltage_Init(void);
  float Drivers_Voltage_getAnalogue(void);
#if _Test_Drivers
  void Drivers_Voltage_Test(void);
#endif
private:
...
{
public:
  void Drivers_Motor_Init(void);
#if _Test_Drivers
  void Drivers_Motor_Test(void);
#endif
  void Drivers_Motor_control(boolean direction_A, uint8_t speed_A, //Group A motor parameters
...
//#include <NewPing.h>
class Drivers_ULTRASONIC
{
public:
  void Drivers_ULTRASONIC_Init(void);
#if _Test_Drivers
  void Drivers_ULTRASONIC_Test(void);
#endif
  void Drivers_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get /*out*/);
...
{
public:
  void Drivers_Servo_Init(unsigned int Position_angle);
#if _Test_Drivers
  void Drivers_Servo_Test(void);
#endif
  void Drivers_Servo_control(unsigned int Position_angle);
  void Drivers_Servo_controls(uint8_t Servo, unsigned int Position_angle);
...
/*IRrecv*/
#include "IRremote.h"
class Drivers_IRrecv
{
public:
  void Drivers_IRrecv_Init(void);
  bool Drivers_IRrecv_Get(uint8_t *IRrecv_Get /*out*/);
  void Drivers_IRrecv_Test(void);
